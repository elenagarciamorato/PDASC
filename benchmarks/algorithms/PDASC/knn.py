...
def PDASC(config_file):
    ...
    logging.info("")
    logging.info('---- PDASC Parameters - tam_grupo=%s - n_centroids=%s - radius=%s - algorithm=%s - implementation=%s ----', tam_grupo, n_centroides, initial_radius, algorithm, implementation)
    logging.info('------------------------------------------------------------------------\n')
    ...
    # with open("./algorithms/PDASC_MNIST" + str(k) +".pickle", 'wb') as handle:
