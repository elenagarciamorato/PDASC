...
def experiment(config_file):
    ...
    if method == 'Exact':
        ...
        FAISS_IVF(config_file)

    elif method == 'LSH':
        FAISS_LSH(config_file)

    elif method == 'FAISSHNSW':
        FAISS_HNSW(config_file)
...
def execute_experiments(argument, optional_filters=None):
    ...
    elif os.path.isdir("./ANN_Experiments/config/" + argument):
        config_files = [f for f in os.listdir("./ANN_Experiments/config/" + argument) if f.endswith('.ini')]
